...
@pytest.mark.smoke
def test_TC01_successful_login(driver, base_url):

    # First, we create an instance of the LoginPage by calling it to allow us to use the methods of LoginPage class
    login_page = LoginPage(driver)
    # We fetch the username and password from environment variables, which are set in the .env file by using the os.getenv() function
    email = os.getenv("REGISTERED_EMAIL")
    password = os.getenv("REGISTERED_PASSWORD")
    login_page.navigate_to_login_page(base_url)
    # Then we launch the actions with login() function that includes every action needed to perform a sucessfull login
...
def test_TC02_failed_login_invalid_email(driver, base_url):
...
@pytest.mark.regression
def test_TC03_failed_login_invalid_password(driver, base_url):
...
@pytest.mark.smoke
def test_TC04_redirect_homepage_after_successful_login(driver, base_url):
...
@pytest.mark.regression
def test_TC05_failed_empty_fields_login(driver, base_url):
