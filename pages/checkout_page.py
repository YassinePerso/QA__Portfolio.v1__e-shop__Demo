...
class CheckoutPage (BasePage):
    ...
    def __init__(self, driver):
        ...
        self.message_successful_order = (
            By.XPATH, "//strong[normalize-space()='Your order has been successfully processed!']")
    ...
    def select_billing_address(self):
        # Using Select() function to select dropdown's options
        dropdown = Select(self.find_element(self.billing_address_input))
        # Select the first option in the list with select_by_index() function - (index[0] = first)
    ...
    def select_shipping_address(self):
        # Using Select() function to select dropdown's options
        dropdown = Select(self.find_element(self.shipping_address_input))
        # Select the first option in the list with select_by_index() function - (index[0] = first)
        dropdown.select_by_index(0)
    ...
    def complete_credit_card_informations(self):
        ...
        # Using Select() function to select dropdown's options
        type_credit_card_dropdown = Select(self.find_element(self.type_credit_card_select_input))
        ...
        # Using Select() function to select dropdown's options
        expiration_month_dropdown = Select(self.find_element(self.expiration_date_month))
        expiration_month_dropdown.select_by_index(0)  # (= January)

        # Using Select() function to select dropdown's options
        expiration_year_dropdown = Select(self.find_element(self.expiration_date_year))
